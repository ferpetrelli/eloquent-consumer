...
class ModelBuilder
{
     ...
     *
     * @var \Petrelli\EloquentConsumer\Builders\QueryBuilder;
     */
    protected $query;
     ...
     *
     * @var \Petrelli\EloquentConsumer\Models\ApiModel
     */
    protected $model;
     ...
     * @var array
     */
    protected $passthru = ['runGet', 'runPost'];
     ...
     *
     * @param  \Petrelli\EloquentConsumer\Models\ApiModel
     * @return $this
     */
     ...
     * @param  array  $columns
     */
    public function execute($columns = [], $verb = 'GET')
    {
        $builder = clone $this;

        switch($verb) {
            case 'GET':
                if (count($models = $builder->retrieveModels($columns, 'GET')) > 0) {
                    $models = $builder->eagerLoadRelations($models);
                }
                break;
            case 'POST':
                if (count($models = $builder->retrieveModels($columns, 'POST')) > 0) {
                    $models = $builder->eagerLoadRelations($models);
                }
                break;
    ...
    /**
     * Execute a GET call and return a collection of results
     *
     * @param  array  $columns
     */
    public function get($columns = [])
    {
        return $this->execute($columns, 'GET');
    }


    /**
     * Execute a POST call and return a collection of results
     *
     * @param  array  $columns
     */
    public function post($columns = [])
    {
        return $this->execute($columns, 'POST');
    }
    ...
    public function getRaw($columns = [])
    {
        ...
        return $this->query->getRaw($columns, $this->getEndpointURL($this->resolveCollectionEndpoint()))->all();
    }
     ...
     * @param  array  $columns
     */
    public function postRaw($columns = [])
    {
        $builder = clone $this;

        return $this->query->postRaw($columns, $this->getEndpointURL($this->resolveCollectionEndpoint()))->all();
    }
     ...
     * @param  array  $columns
     */
    public function retrieveModels($columns = [], $verb = 'GET')
    {

        switch ($verb) {
            case 'GET':
                $results = $this->query->get($columns, $this->getEndpointURL($this->resolveCollectionEndpoint()));
                break;
            case 'POST':
                $results = $this->query->post($columns, $this->getEndpointURL($this->resolveCollectionEndpoint()));
                break;
        }
     ...
     * @param  array  $columns
     */
    public function getSearch($perPage, $columns = [], $pageName = 'page', $page = null)
    {
        $builder = clone $this;
    ...
    {
        $builder = clone $this;
        $endpoint = $this->getEndpointURL($this->resolveResourceEndpoint(), ['id' => $id]);
     ...
     */
    public function getPaginated($perPage, $columns = [], $pageName = 'page', $page = null)
    {
        if (is_null($perPage)) {
     ...
     * @return string
     */
    public function getEndpointURL($name, $params = [])
    {
        return $this->model->getEndpoint()->parseEndpoint($name, $params);
    }
    ...
    /**
     * Resolve endpoint for a collection.
     * Search and listings contains could be different.
     * We will check if we are calling a search
     * and use that endpoint in that case
     *
     * @return string
     */
     ...
     * Get the model instance being queried.
     *
     * @return \Petrelli\EloquentConsumer\Models\ApiModel;
     */
